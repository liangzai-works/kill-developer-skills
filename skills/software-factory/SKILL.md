...
description: |
  ...
  详细设计文档会按 3 级优先级读取 "详细设计模板.docx" 作为样式基线 (若无则 fallback)。
...
| 2 | 详细设计 | ✅ | 02_<项目名>_详细设计文档.docx (按 §5 优先级找模板) |
...
- v0.3.0: 修复详细设计模板查找路径错误
  - 旧版只在 `<项目名>工作空间/` 下找, 但该目录是产物目录, 永远不会有模板
  - 新版按 4 级优先级查找 (显式参数 → 项目级覆盖 → skill 自带 → docx 默认)
  - 模板作为 skill 资源放到 `skills/software-factory/templates/`, 并加入 `.gitignore` (用户私有资产)
  - 新增 `--template=<绝对路径>` 显式传入入口
